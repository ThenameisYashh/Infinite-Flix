...
fetch(`https://6751855ed1983b9597b336b6.mockapi.io/all/${idnum}`)
  ...
  .then((data) => {
    ...
    document.querySelector(".fstD").innerHTML = `<label>${data.Title.split(
      "_"
    ).join(" ")}</label><span>${data.heading}</span><p class='epPlot'>${
      data.Plot
    }</p>`;
    document.querySelector(
      ".scnD"
...
function rscrl() {
  epContainer.scrollLeft += rarr.parentNode.children[1].children[0].clientWidth + 12;
...
function rscrl2() {
  ep2Container.scrollLeft += rarr.parentNode.children[1].children[0].clientWidth + 12;
...
function rscrl3() {
  ep3Container.scrollLeft += rarr.parentNode.children[1].children[0].clientWidth + 12;
